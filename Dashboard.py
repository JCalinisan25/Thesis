...
def profile():
    ...
    close = Button(f2, bg="#010F57", bd=0, height=1, width=1, text="X", fg='white', font=('Arial', 15, 'bold'),
                   cursor='hand2', command=exit, activebackground='#010F57')
    ...
    select_btn = Button(f2, text= 'Change Profile', fg="white",font=('Arial', 9, 'bold', 'underline'), 
                        bg='#010F57' , bd=0, cursor="hand2", command=select_image, width=15, 
                        activebackground='#010F57', activeforeground='#C64CCD')
    select_btn.place(x=155, y=118)
    ...
    def change_credentials():
        ...
        try:
            ...
            password_hash = user['users'][0]['passwordHash']
            auth._firebase_auth.update_user(user_id, password=new_password, password_hash=password_hash)
            messagebox.showinfo("Success", "Username and password have been updated.")
            ...
            # Display the selected profile image on the person label
                person_lbl.config(image=profile_image)
                person_lbl.image = profile_image
    ...
    new_username_entry.place(x=180, y=150)

    # Password label and entry
    password_label = Label(f2, text="New Password:", fg='white', bg='#010F57', font=('Arial', 12, 'bold'))
    password_label.place(x=40, y=190)

    new_password_entry = Entry(f2, width=30, show="*")
    new_password_entry.place(x=180, y=190)

    # Confirm Password label and entry
    confirm_password_label = Label(f2, text="Confirm Password:", fg='white', bg='#010F57', font=('Arial', 12, 'bold'))
    confirm_password_label.place(x=20, y=230)

    confirm_password_entry = Entry(f2, width=30, show="*")
    confirm_password_entry.place(x=180, y=230)
    ...
    change_credentials_btn.place(x=180, y=270)
...
person_lbl = Label(box_2, image=per_pk, border=0)
person_lbl.place(x=160, y=15)

edit_btn = Image.open("img\\pencil.png")
edit_pk = ImageTk.PhotoImage(edit_btn.resize((15, 15)))

edit_lbl = Label(image=edit_pk)

brgr = Image.open("img\hamburger.png")
brgr_pk = ImageTk.PhotoImage(brgr.resize((35, 35)))

lbl2 = Label( image=brgr_pk)

brgr_btn = Button(box_2, bg="#010F57", bd=0, height=40, width=30, image=brgr_pk, cursor='hand2', command=toggle_menu, activebackground='#010F57')
brgr_btn.place(x=10, y=5)
...
lbl3 = Label( image=lg_pk)

lg_btn = Button(box_2, bg="#010F57", bd=0, height=40, width=30, image=lg_pk, cursor='hand2', command=tosign, activebackground='#010F57')
lg_btn.place(x=370, y=5)
